...
function createRoomSubscription(roomName) {
  ...
  consumer.subscriptions.create(
    { channel: "ChatChannel", room: roomName },
    {
      ...
      createLine(data) {
        return `
          <article id="message_${data["message_id"]}" class="mb-4">
            <div class="flex items-baseline gap-3">
              <div class="h-8 w-8 rounded-full bg-gray-300 flex items-center justify-center text-sm font-medium">
                ${data["user_image"]}
              </div>
              <div>
                ...
                <div class="mt-1 text-sm text-slate-700">${data["message_content"]}</div>
              </div>
            </div>
          </article>
        `;
